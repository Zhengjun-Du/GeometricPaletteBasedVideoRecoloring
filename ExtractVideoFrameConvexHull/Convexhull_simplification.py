...
def write_convexhull_into_obj_file(hull, output_rawhull_obj_file):
    ...
    for index in range(hvertices.shape[0]):
        ...
        #dzj clip
        if hvertices[index][0] > 1.0 :
          hvertices[index][0] = 1.0
        if hvertices[index][0] < 0 :
          hvertices[index][0] = 0 
        if hvertices[index][1] > 1.0 :
          hvertices[index][1] = 1.0
        if hvertices[index][1] < 0 :
          hvertices[index][1] = 0 
        if hvertices[index][2] > 1.0 :
          hvertices[index][2] = 1.0
        if hvertices[index][2] < 0 :
          hvertices[index][2] = 0 
